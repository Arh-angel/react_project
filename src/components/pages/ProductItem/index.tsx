...
const ProductItem = (props: ProductItemPropsType) => {
  const {
    chengeTotal, productName, productDescription, price, action
  } = props;
  ...
  const prevTotalPrice = prevPriceRef.current;
  ...
  useEffect(() => {
    if (totalPrice < prevTotalPrice) {
      return chengeTotal(price * -1);
    }
    return chengeTotal(price);
  }, [totalPrice]);

  return (
    <div className={style.ProductItemContainer}>
      <div className={style.ProductItemImg}>
        <img src="productImg.png" alt="product-img" />
      </div>
      <div className={style.ProductItemSetting}>
        <div className={style.ProductItemDescription}>
          <h3 className={style.ProductItemTitle}>{ productName }</h3>
          <p className={style.ProductItemText}>{ productDescription }</p>
        </div>
        <div className={style.ProductItemNumberPrice}>
          <div className={style.ProductItemNumber}>
            <button type="button" onClick={minusNumHeandler} className={style.ProductItemNumberBtn}>
            ...
            <p className={style.ProductItemNumberFig}>{ number }</p>
            <button type="button" onClick={plusNumHeandler} className={style.ProductItemNumberBtn}>
        ...
        <div className={style.ProductItemTotalPay}>
          <div className={style.ProductItemTotalPayWrapper}>
            <p className={style.ProductItemTotal}>
              Итого:
              { ' ' }
              { totalPrice }
              { ' ' }
            ...
            <div className={style.ProductItemPayWrapper}>
              <button type="button" className={style.ProductItemPay}>
                { action }
